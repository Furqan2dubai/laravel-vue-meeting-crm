...
axios.get("/api/meeting/" + id).then(res=>{
  if(res.data.success){    
    Meeting.value = res.data.data[0]  
    // delete Meeting.value.items
    } 
  else{
      store.setMsgbox({status:false, msg:"Failed to load data"}); 
  }
...
const update = ()=>{  
  delete Meeting.value.items;
  axios.post("/api/meeting/"+id , {Meeting:Meeting.value}).then(res=>{
  if(res.data.success){   
...
<template>
  <VForm @submit.prevent="update" enctype="multipart/form-data">
  <VRow>  
    <Msgbox />  
    <VCol cols="12">
      <!-- 👉 Multiple Column -->
      <VCard title="Edit Meeting">
